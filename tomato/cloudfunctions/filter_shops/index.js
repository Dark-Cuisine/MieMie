...
exports.main = async (event, context) => {
  ...
  shopKind &&
    !(
      shopKind.shopKindSmall ==
      classifications.shopKinds.shopKindSmall[0].shopKindSmall[0]
    ) &&
    filterOptionAndList.push({
      shopInfo: {
        shopKinds: {
  ...
  if (filterOptionAndList.length > 0) {
    return await db.collection("shops").where(_.and(filterOptionAndList)).get();
  } else {
    return await db.collection("shops").get();
  }
