...
const ManageShopPage = (props) => {
  ...
  const deleteShop = async () => {
    ...
    let deletedImg = state.shop.shopInfo.shopIcon.concat(
      state.shop.shopInfo.QRCodeList,
      deletedProductIcons
    );
    let deletedUrl = [];
    ...
    deletedUrl.length > 0 && deleteImgs(deletedUrl);

    await databaseFunctions.shop_functions.deleteShop(
      state.shop,
      userManager.unionid
    );
  ...
  const uploadShop = async () => {
    ...
    deletedImgList.productIcons &&
      deletedImgList.productIcons.length > 0 && //unfinished 好像一开始直接放一起就好....
      deleted.push(...deletedImgList.productIcons);
    deletedImgList.shopIcons &&
      deletedImgList.shopIcons.length > 0 &&
      deleted.push(...deletedImgList.shopIcons);
    deletedImgList.qrCodes &&
      deletedImgList.qrCodes.length > 0 &&
    ...
    if (state.deletedProducts && state.deletedProducts.length > 0) {
      deletedProducts.forEach((p) => {
        ...
        if (p.icon && p.icon.length > 0) {
          p.icon.fileID && deleted.push({ url: p.icon.fileID });
        }
      });
    }
    ...
    for (let it of state.shop.shopInfo.shopIcon) {
      let updated = it.fileID
        ? it
        : await databaseFunctions.img_functions.compressAndUploadImg(
            it,
            fileDir,
            "shop_icons"
          );
      ...
      if (state.way == "ADD") {
        await databaseFunctions.shop_functions.addNewShop(
          userManager.unionid,
          state.shop,
          updatedProductList
        );
      } else {
        await databaseFunctions.shop_functions.modifyShop(
          updatedShop,
          updatedProductList,
          state.deletedProducts
        );
      }
      await databaseFunctions.user_functions.updatePaymentOptions(
        userManager.unionid,
  ...
  //save ref-value
  const handleSave = (
    newTab = state.currentTab,
    oldTab = state.currentTab,
    isUploading = false
  ) => {
    //isUploading用来判断是否打开确认上传店铺的dialog
    ...
    switch (oldTab) {
      case 0: //SHOP_INFO
        ...
        setDeletedImgList({
          ...deletedImgList,
          shopIcons:
            value.deletedImgList.shopIcons &&
            value.deletedImgList.shopIcons.length > 0
              ? [...deletedImgList.shopIcons, ...value.deletedImgList.shopIcons]
              : [],
          qrCodes:
            value.deletedImgList.qrCodes &&
            value.deletedImgList.qrCodes.length > 0
      ...
      case 2: //PRODUCT
        ...
        setDeletedImgList({
          ...deletedImgList,
          productIcons:
            value.deletedImgList.productIcons &&
            value.deletedImgList.productIcons.length > 0
              ? [
                  ...deletedImgList.productIcons,
                  ...value.deletedImgList.productIcons,
                ]
