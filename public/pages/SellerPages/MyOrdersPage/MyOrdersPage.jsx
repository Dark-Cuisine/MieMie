...
class MyOrdersPage extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      !(nextProps.userManager.unionid == this.props.userManager.unionid) ||
      (!(
        nextProps.layoutManager.currentTabId ==
        this.props.layoutManager.currentTabId
      ) &&
        nextProps.layoutManager.currentTabId ==
  ...
  doUpdate(props = this.props) {
    ...
    if (
      !(
        props.userManager.unionid &&
        props.userManager.unionid.length > 0 &&
        props.userManager.userInfo
      )
    ) {
      return;
    }
        ...
        if (orderIdList.length > 0) {
          wx.cloud.callFunction({
            ...
            success: (r) => {
              ...
              if (
                !(r && r.result && r.result.data && r.result.data.length > 0)
              ) {
                props.toggleLoadingSpinner(false);
                return;
              }
              ...
              this.setState({
                //*注：must setState inside .then() or it will get empty []!
                ...this.state,
                myShops: myShops,
                ordersReceived: {
  ...
  sengOrderMsg = async (type, order) => {
    ...
    switch (type) {
      ...
      case "ORDER_REJECTED":
        ...
        content =
          ...
          (order.rejectedReason.des.length > 0 &&
            " : " + order.rejectedReason.des);
        break;
      case "ORDER_FINISHED":
        title = "摊主已完成订单";
    ...
    await databaseFunctions.msg_functions.sendMessage(
      msg,
      this.props.userManager.unionid
    );
  ...
  handleSubmit = async (way, i = null) => {
    ...
    switch (way) {
      case "ACCEPT_FROM_UNPROCESSED":
        ...
        if (!(order && order._id && order._id.length > 0)) {
          return;
        }
  ...
  render() {
    ...
    let rejectDialogInput = (
      <View className="">
            ...
            return (
              <View
                ...
                className={"mie_button ".concat(
                  it == this.state.rejectReason.reason
                    ? "mie_button_choosen"
                    : ""
                )}
        ...
        <AtTextarea
          ...
          onChange={(value) =>
            this.handleChangeInput("REJECT_REASON_DES", value)
          }
        />
      </View>
    );
    let rejectOrderDialog = (
      <ActionDialog
        ...
        closeOnClickOverlay={
          !(
            this.state.rejectReason.des &&
            this.state.rejectReason.des.length > 0
          )
        }
    ...
    let finishOrderDialog = (
      ...
      >
        <View>
          点击完成后，该单在买家处也会标记为已完成，请确保交易已结束。你确定要现在完成该订单?
        </View>
      </ActionDialog>
    );
          ...
          return (
            <View key={it._id}>
              <OrderCard
                ...
                handleClickButtonLeft={this.handleBeforeSubmit.bind(
                  this,
                  "REJECT",
                  i
                )}
                handleClickButtonRight={this.handleSubmit.bind(
                  this,
                  "ACCEPT_FROM_UNPROCESSED",
                  i
                )}
              />
            </View>
          ...
          return (
            <View key={it._id}>
              <OrderCard
                ...
                handleClickButtonLeft={this.handleBeforeSubmit.bind(
                  this,
                  "REJECT_FROM_ACCEPTED",
                  i
                )}
                handleClickButtonRight={this.handleBeforeSubmit.bind(
                  this,
                  "FINISH",
                  i
                )}
            ...
            <View key={it._id}>
              <OrderCard
                ...
                handleClickButtonRight={() =>
                  this.handleSubmit("DELETE_FROM_REJECTED", i)
                }
