...
const SearchBar = (props) => {
  ...
  const doSearch = async () => {
    ...
    db_1
      ...
      .then((res) => {
        dispatch(
          actions.setSearchedProductList(res && res.data ? res.data : [])
        );
        dispatch(actions.toggleLoadingSpinner(false));
      });
  };
  ...
  const handleChangeSearchBar = (v) => {
    ...
    v.length > 0 ? props.toggleSearchBar(true) : props.toggleSearchBar(false);
  };

  const handleBlur = () => {
    //console.log('onb');
    state.searchBarValue.length < 1 && props.toggleSearchBar(false);
  };
